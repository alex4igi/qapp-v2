-- Open class — permite recepției să depășească limita sesiunii pentru walk-in.
-- Limita (capacitate) rămâne STRICTĂ pentru rezervările online din portalul de membru
-- (hold_loc_open nu primește acest flag). La recepție (rezerva_loc_open), staff-ul poate
-- adăuga un client peste limită cu un flag explicit `p_permite_overbook`.
--
-- Adăugăm un parametru nou cu DEFAULT => apelurile existente (9 argumente) rămân valide.
-- Dropăm întâi semnătura veche ca să nu rămână două overload-uri ambigue pentru PostgREST.

drop function if exists rezerva_loc_open(uuid, numeric, metoda_plata, uuid, uuid, uuid, date, uuid, date);
...
grant execute on function rezerva_loc_open(uuid, numeric, metoda_plata, uuid, uuid, uuid, date, uuid, date, boolean)
  to authenticated;
