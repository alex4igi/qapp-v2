-- pg_cron la fiecare minut → invoca edge fn process-review-sms, care dreneaza
-- coada confirmari_review_sms (delay de 5 min pe send_after). Cheia anon e
-- publica (RLS protejeaza datele; functia are verify_jwt=false).
create extension if not exists pg_net;

select cron.unschedule(jobid)
  from cron.job where jobname = 'process-review-sms';
...
select cron.schedule(
  ...
  select net.http_post(
    url := 'https://cbftxkwvoboqahzsldcp.supabase.co/functions/v1/process-review-sms',
