-- Extinde clone_sezon cu scadențele primei/ultimei rate (vezi 20260608150000).
-- Wizard-ul de clonare le poate seta direct la creare, ca să nu fie uitate.
-- Adăugăm 2 parametri (cu default null) → semnătură nouă, deci DROP + CREATE.

drop function if exists clone_sezon(uuid, text, text, date, date, jsonb, jsonb);

create function clone_sezon(
  p_sezon_sursa uuid,
  p_nume text,
  p_tip text,
