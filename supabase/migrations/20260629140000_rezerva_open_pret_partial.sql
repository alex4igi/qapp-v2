-- Open class — încasare parțială / 0 lei la rezervare (clientul devine restant).
--
-- Până acum `enrollments.suma` = suma încasată (p_suma + p_suma2), deci prețul real
-- și suma plătită erau confundate → nu se putea rezerva un loc plătind 0/parțial cu
-- restul urmărit ca restanță. Separăm prețul (p_pret) de încasat (p_suma/p_suma2):
--
--   • enrollments.suma_baza / enrollments.suma  ← p_pret   (cât datorează)
--   • open_rezervari.suma                        ← p_pret
--   • incasari                                   ← p_suma (+ p_suma2)  (cât a plătit acum)
--   • rest = p_pret − (p_suma + p_suma2)  →  iese automat în plati_inrolari
--
-- p_pret are DEFAULT null → apelurile vechi (fără p_pret) păstrează exact
-- comportamentul anterior: v_pret = p_suma + coalesce(p_suma2, 0).
--
-- Permitem p_suma = 0: în acest caz NU se creează nicio încasare, iar rezervarea
-- rămâne fără încasare legată (incasare = null). Locul e totuși rezervat.

drop function if exists rezerva_loc_open(uuid, numeric, metoda_plata, uuid, uuid, uuid, date, uuid, date, boolean, metoda_plata, numeric);
...
grant execute on function rezerva_loc_open(uuid, numeric, metoda_plata, uuid, uuid, uuid, date, uuid, date, boolean, metoda_plata, numeric, numeric)
  to authenticated;
