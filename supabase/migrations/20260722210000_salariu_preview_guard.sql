-- Gard pe `calculeaza_salariu_teacher`: doar adminul/owner-ul, instructorul insusi
-- sau `service_role` pot calcula un salariu. Necesar acum, cand „Salariul meu" incepe
-- sa ceara preview live (nu doar snapshot-uri confirmate) — RPC-ul era `security
-- definer` acordat lui `authenticated`, fara nicio verificare de rol.
--
-- Corpul functiei e neatins fata de 20260708190000 (praguri, regula 90%, scop pe
-- sezon) — modelul de salarizare ramane inghetat pana la definirea celui nou.
