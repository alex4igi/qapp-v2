-- O suspendare programată într-o lună viitoare nu se putea retrage: butonul cerea
-- re-activare, iar re-activarea trebuia să fie DUPĂ luna suspendării, deci
-- programarea rămânea blocată în viitor. Re-activarea din chiar luna suspendării
-- devine ANULARE — intervalul se șterge, grupa n-a fost oprită nicio lună.
-- Restul funcției e neschimbat față de 20260913182000.
...
begin
  ...
  if p_suspenda then
    ...
    -- Re-activarea din CHIAR luna suspendării = anulare: grupa n-a fost oprită
    -- nicio lună, deci intervalul dispare în loc să rămână gol. Așa se retrage o
    -- suspendare programată greșit într-o lună viitoare, singura cale de ieșire
    -- dintr-o programare care altfel nu s-ar mai putea desface.
    if v_luna = v_deschisa.din_luna then
      delete from cursuri_suspendari where id = v_deschisa.id;
    elsif v_luna < v_deschisa.din_luna then
      raise exception 'Re-activarea nu poate fi dinaintea suspendării (%).',
