-- Conversie „ședințe → abonament", rescrisă pe lună (înlocuiește
-- converteste_sedinta_in_abonament, care lucra pe o singură ședință).
--
-- Bug-ul vechiului flux (client 52c76b57, 28 iul 2026): butonul converteia exact
-- rândul pe care s-a apăsat, fără gardă de lună — o ședință din 30 iunie a ajuns
-- să finanțeze abonamentul de iulie (bani + prezență mutate dintr-o lună închisă),
-- iar celelalte 3 ședințe din iulie au rămas rânduri separate lângă abonament
-- (460 RON facturați pe iulie în loc de 260).
--
-- Model nou: conversia e „clientul trece pe abonament în luna X la cursul Y".
--   (a) creează UN abonament „Per luna" pe luna aleasă (preț din curs; triggerul
--       de politică aplică reducerea de pool dacă e cazul);
--   (b) mută pe el încasările TUTUROR ședințelor active din acea lună;
--   (c) mută prezențele lor (invariantul 1 prezență/(client,curs,zi) e păstrat:
--       ședințele au date distincte, iar abonamentul e nou);
--   (d) zerează rezervările OPEN (rămân active → roster/prezență păstrate);
--   (e) void curat al ședințelor (suma = suma_baza = 0, reziliat) → fără restanță
--       fantomă (view-urile de restanțe exclud reziliat = true).
--
-- Ședințele din alte luni nu se ating. Prețul e pe luna întreagă: ce s-a plătit deja
-- pe ședințele lunii devine avans pe abonament (decizie business, 29 iul 2026).
--
-- Authz: front_desk și mai sus (recepția face conversia la ghișeu).

drop function if exists converteste_sedinta_in_abonament(uuid, uuid, text);
...
begin
  ...
  insert into enrollments (
    ...
    v_luna_start, null, true, v_sezon
  )
