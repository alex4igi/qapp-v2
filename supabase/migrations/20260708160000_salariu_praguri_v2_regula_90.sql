-- Salariu teacher v2: (1) praguri finale corectate, (2) regula 90% pe ramura „a plătit".
--
-- Corecții praguri (confirmate user 2026-07-08):
--   Per prezențe (facultativ): ≥275→2250, ≥250→2000, ≥200→1500, ≥160→1280,
--                              ≥140→1000, ≥120→840, ≥100→600, ≥80→400, else 0.
--   Per client (recurent):     ≥25→2500, ≥23→1500, ≥20→1280, ≥17→1000, ≥15→840,
--                              ≥12→600, ≥10→400, 1-9→400 (floor), 0→0. 1 ședință=½.
--
-- Regula 90% (ramura „a plătit" din numărarea cursanților recurenți):
--   Un client vechi (data_incepere < luna) se numără doar dacă a încasat în lună
--   ≥ 90% din valoarea LUNARĂ a abonamentului. Valoarea = `enrollments.suma`, care
--   include deja voucher + discount familie/cross-sell → clientul cu astfel de
--   discount plătește ~100% din suma lui redusă și E numărat. Cei veniți târziu
--   (prorata) sau noi cu 50% plătesc o fracțiune → sub 90% → nu se numără.
--   Pt înrolări „Per an" (un rând pe sezon) împărțim suma la nr. de luni acoperite
--   ca referința să fie lunară, nu anuală. Ramura „>2 prezențe" rămâne OR (neschimbată).
...
begin
  ...
  loop
    ...
    -- fereastra lunii care a PLĂTIT ≥90% din valoarea lunară a abonamentului
    -- (ramură care exclude prorata & discount 50%, dar NU familie/cross-sell)
    -- SAU a avut >2 prezențe în lună.
    with eligibili as (
        ...
        and (
          (
            ...
            and (
              ...
              -- valoarea LUNARĂ: suma / nr. luni acoperite (Per an → /10, Per lună → /1)
    ...
    elsif v_nr_cursanti >= 25 then v_suma_client := 2500; v_prag_client := 25;
