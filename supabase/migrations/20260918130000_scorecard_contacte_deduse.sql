-- Scorecard: contactul DEDUS din mutarea cardului intră la muncă, dar se vede separat.
--
-- Vezi 20260918120000_contact_dedus_din_mutare.sql: butonul „Loghează contact"
-- era apăsat pe ~16% din leaduri, deși aproape toate fuseseră lucrate. Acum
-- aplicația scrie singură contactul când cineva mută cardul, iar aici:
--   `contacte_logate`  = apăsate explicit  → disciplina de CRM
--   `contacte_deduse`  = deduse din mutare → munca pe care n-o vedeam
--   `contacte_total`   = suma lor
-- Igiena CRM (nota scrisă) și detectorul de „rafală" rămân DOAR pe logările
-- explicite: nota triggerului nu e o notă, iar o mutare în bloc de carduri nu e
-- o rafală de apeluri. Semnătura funcției se schimbă → drop întâi.

drop function if exists get_scorecard_operatori(date, date, text);
...
returns table (
  ...
  contacte_logate     integer,
  contacte_deduse     integer,
...
as $$
  ...
  ci as (
    select lc.id, lc.lead_id, lc.user_id, lc.canal, lc.rezultat, lc.observatii, lc.created, lc.dedus
  ...
  lm as (
    select lo.user_id as owner, l.id as lead_id, l.status, l.data_conversie,
           ...
           exists (select 1 from ci where ci.lead_id = l.id and not ci.dedus and ci.observatii is not null and length(trim(ci.observatii)) > 0) as are_nota
    from lead_owner lo
  ...
  vol as (
    select
      ...
      count(*) filter (where not ci.dedus)::int as contacte_logate,
      count(*) filter (where ci.dedus)::int as contacte_deduse,
      ...
      round(
        100.0 * count(*) filter (where not ci.dedus and ci.rezultat = 'reusit' and (ci.observatii is null or length(trim(ci.observatii)) = 0))
        / nullif(count(*) filter (where not ci.dedus and ci.rezultat = 'reusit'), 0)
      , 1) as nota_lipsa_pct
    from ci
  ...
  metrics as (
    select
      ...
      coalesce(v.contacte_logate, 0) as contacte_logate,
      coalesce(v.contacte_deduse, 0) as contacte_deduse,
