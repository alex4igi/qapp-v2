-- get_sms_recipients: `reminder_plata` rămâne deschis și după termen, până în ziua
-- dinaintea mesajului de datorii.
--
-- Calendarul trimiterilor (decis 2026-09-21) stă la distanță fixă de termenul
-- lunii: reminder cu 5 zile înainte, datorii cu 10 zile după. Lunile obișnuite:
-- 10 și 25; septembrie (termen 20): 15 și 30; iunie (termen 7): 2 și 17.
-- Între termen și ziua datoriilor, reminderul pleacă în varianta „termenul a
-- fost …" (textul îl alege composer-ul, după `scadenta`). Motivul: în fereastra
-- asta, restanța din DB include și transferurile făcute la timp, dar încă
-- neimportate din extras — un mesaj de „sold restant" le-ar nimeri greșit.
-- Oglinda în TS: src/features/notificari-sms/calendar.ts (ZILE_DATORII_DUPA).
--
-- Corpul e cel din 20260831170200; se schimbă doar fereastra `reminder_plata`.
