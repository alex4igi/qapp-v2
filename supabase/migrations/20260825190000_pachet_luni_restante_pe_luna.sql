-- Pachetul de luni (/analytics) — tile-ul „Restanțe (>7z peste scadență)" era
-- citit din trei baze diferite pe același rând: suma și familiile pe soldul
-- CUMULAT (toate lunile neprescrise), iar procentul doar pe luna curentă. Adică
-- „301 familii · 14,7% din facturarea lunii" nu descriau aceeași mulțime.
--
-- Decizie user 2026-08-25: tot tile-ul pe LUNA CURENTĂ (suma, familiile și
-- procentul din aceeași bază). Soldul istoric rămâne pe /datorii (nota de subsol)
-- și în worklist. Metrica rămâne cea proprie — „>7 zile peste scadență" — deci
-- între 1 și 22 ale lunii tile-ul arată 0 (nimic nu e încă întârziat), exact cum
-- se comporta deja procentul; acum toate cele trei cifre spun același lucru.
--
-- Snapshot-urile 'prev' (−7 zile) și 'yoy' (−364 zile) folosesc luna în care
-- cădea data lor, nu luna de azi.
-- Numărătorul procentului nu se mai calculează separat în fact_luna: e chiar
-- suma afișată în tile, deci nu mai pot diverge.
--
-- Semnătură neschimbată (get_pachet_luni(uuid)).
