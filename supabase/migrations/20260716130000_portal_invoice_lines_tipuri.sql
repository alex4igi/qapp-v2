-- Descrieri factură portal PE TIP DE SERVICIU (fără numele cursului), aliniate la
-- nomenclatorul FGO existent (ARTICOLE_FGO din src/features/facturare/constants.ts),
-- ca facturile din portal să fie consecvente cu cele emise manual din bancă.
--
-- Mapare:
--   enrollment.tip_plata:
--     'Per sedinta' → „Sedinta dans gimnastica"
--     'Per luna'    → „Abonament dans gimnastica — <luna anul>"
--     'Per an'      → „Abonament anual dans gimnastica <anul>"
--   datorie one-off după categorie (+ text / FK):
--     Merch      → „Articole vestimentar [— <produs>]"    (inventar.articol)
--     Bilet      → „Bilet <eveniment>"                     (datorii.bilet→evenimente)
--     Workshop   → „Taxa workshop"
--     Auditie    → „Taxa auditie"
--     Inchiriere → descrierea închirierii
--     Taxa       → Reînscriere / concurs / confirmare loc din text, altfel textul liber
--   order_type='rezervare' (open class) → „Sedinta dans gimnastica din <dată>"
--   order_type='bilet'                  → „Bilet <eveniment> [× N]"
...
begin
  ...
  loop
    ...
    -- Element = înrolare (recurent / ședință / anual): etichetă după tip_plata.
    ...
    -- Element = datorie one-off: etichetă după categorie (+ text / FK).
