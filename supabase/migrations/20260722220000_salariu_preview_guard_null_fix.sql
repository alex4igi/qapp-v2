-- Fix la garda din 20260722210000: comparatia `p_teacher = my_teacher_id()` da NULL
-- (nu false) pentru conturile fara profil de instructor, iar `not (false or NULL)`
-- e NULL → `if` nu se aprinde → garda NU bloca. Efect: orice cont `authenticated`
-- fara profil legat (front_desk, staff nelegat) putea calcula salariul oricui.
-- Verificat cu cont real inainte si dupa fix.
