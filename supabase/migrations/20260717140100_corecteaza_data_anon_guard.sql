-- Gard anti-anon pentru corecteaza_data_inrolare.
-- Capcană descoperită la smoke-test: auth_role() face fallback pe 'front_desk'
-- când JWT-ul n-are app_metadata.role, iar grantul implicit Supabase pe funcții
-- include rolul `anon` → un apel cu cheia publică trecea de gardul de rol.
-- (⚠️ capcana e sistemică — orice RPC care permite front_desk e afectat la fel;
-- de auditat separat.) Aici: cerem sesiune autentificată + revocăm anon explicit.
...
revoke execute on function corecteaza_data_inrolare(uuid, date, text) from anon;
