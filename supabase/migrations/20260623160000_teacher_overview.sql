-- Qapp v2 — Statistici: overview per instructor (secțiunea „Privire pe instructor").
-- Întoarce un rând per CURS al teacher-ului (sezon activ), pe luna curentă:
--   activi    = clienți activi care acoperă luna curentă (snapshot pe acoperire lună)
--   prezenti  = prezențe distincte însumate pe ședințele ținute luna asta (doar recurent+trupă)
--   posibile  = roster însumat pe acele ședințe (0 la facultativ → UI afișează doar count)
--   datorie   = rest NEPRESCRIS (suma datorată − plătit) pe înrolările ne-reziliate ale cursului
--
-- Convenții reutilizate:
--   prezenti/posibile: identic cu get_rata_prezenta_luna (20260610140000) — rata reală =
--     prezenți distincți / roster pe ședință ținută; absențele nu se marchează.
--   prescriere: data_incepere < current_date - interval '2 years' (20260622110000).
--   teacher→cursuri: c.teacher legacy + cursuri_teacheri M:N (gaura de backfill pe clone).
...
as $$
  ...
  -- Cursurile teacher-ului în sezonul activ: titular legacy SAU rol în M:N.
  cursuri_teacher as (
