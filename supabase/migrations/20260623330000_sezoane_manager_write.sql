-- Qapp v2 — Manager poate crea/clona/edita sezoane (aliniere RLS + RPC cu UI).
-- UI-ul afișează butoanele de sezon nou / clonare pentru manager, dar atât
-- INSERT-ul direct pe `sezoane` (creare manuală) cât și RPC-ul clone_sezon
-- treceau prin is_admin() (doar admin/owner) → managerul primea eroare.
-- Aliniem cu precedentul `cursuri_manager_write`: manager primește INSERT+UPDATE.
-- DELETE rămâne doar pentru admin/owner (via sezoane_admin_all) — distructiv.

-- 1) RLS sezoane: manager poate insera/edita sezoane (creare manuală)
create policy sezoane_manager_insert on sezoane
  for insert to authenticated
  with check (auth_role() in ('admin', 'owner', 'manager'));

create policy sezoane_manager_update on sezoane
  for update to authenticated
  using (auth_role() in ('admin', 'owner', 'manager'))
  with check (auth_role() in ('admin', 'owner', 'manager'));

-- 2) clone_sezon: înlocuiește gardul is_admin() cu unul inclusiv-manager.
--    Restul corpului identic cu 20260608160000_clone_sezon_scadente.sql.
