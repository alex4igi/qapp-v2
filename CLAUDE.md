...
**Regulă GRANT explicit la tabel nou (de la 30 oct. 2026):** Supabase nu mai dă automat drepturi de Data API pe tabelele noi din `public` — un tabel creat fără GRANT răspunde `permission denied` din aplicație (tabelele existente nu sunt afectate). Orice migrație care face `create table` scrie în aceeași migrație granturile, **nu** șablonul din emailul Supabase (acela dă `select` lui `anon`): `grant select, insert, update, delete on public.<t> to authenticated; grant all on public.<t> to service_role;` și **fără nimic pentru `anon`**, cu excepția tabelelor asumat publice (vezi regula de citire anonimă). Dacă tabelul e doar pentru cron/edge function, doar `service_role`. RLS-ul și gardurile `deny_parinte_direct`/`deny_marketing_*` rămân obligatorii — grantul doar deschide ușa, RLS-ul decide ce vezi. Verificare: `node scripts/check-drepturi-tabele.mjs` prinde și tabelul fără grant; tabelele asumat doar-server stau pe lista `DOAR_SERVER` din script.
